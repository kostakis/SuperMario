...
/*Const AFTER function name means that the function WONT modify any members of the class*/
...
public:
	...
	const Rectangle& GetFrameBox(unsigned int frameNo) const {
		//assert(boxes.size() > frameNo);
		return boxes[frameNo];
	}

	void DisplayFrame(); //TODO
